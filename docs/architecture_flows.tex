...
glicose antes de a \texttt{comm\_thread} escolher o perfil. A intenção é
apenas oferecer um fluxo ``reconhecível como parecido com Dexcom'' para fins
de demonstração, mantido atrás de uma opção e com o CGMS da SIG como padrão;
compatibilidade completa com um transmissor real não é um objetivo deste
trabalho.
\guilherme{Se o perfil Dexcom não for demonstrado na banca, considerar mover esta subseção para um apêndice.}
...
pressão: o conceito vem da literatura de CGM, a forma da curva é uma escolha
de projeto.
\guilherme{Adicionar citação para ``compression low'' / artefato por compressão (Facchinetti et al. ou revisão de acurácia de CGM) na descrição clínica do PISA.}
