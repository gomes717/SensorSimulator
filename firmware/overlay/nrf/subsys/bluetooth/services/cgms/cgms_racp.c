...
/* Every instance's RACP jobs are dispatched onto this single shared work
 * queue rather than one queue per instance: RACP requests are low-frequency
 * and already serialized per-instance via each instance's racp_lock, so a
 * dedicated 2KB-stack thread per virtual sensor would be wasted RAM.
 */
static struct k_work_q racp_work_q;
static bool racp_work_q_started;

static struct cgms_meas_db_entry *entry_alloc(struct bt_cgms *cgms)
{
	for (int i = 0; i < ARRAY_SIZE(cgms->racp_pool); i++) {
		if (!cgms->racp_pool[i].used) {
			cgms->racp_pool[i].used = true;
			return &cgms->racp_pool[i];
		}
	}
	return NULL;
}

static int generic_handler(struct bt_conn *peer, struct bt_cgms *cgms,
			uint8_t opcode, uint8_t response_code)
{
	...
	return cgms_racp_send_response(peer, cgms, &rsp);
}

static int report_recs_all_handler(struct bt_conn *peer, struct bt_cgms *cgms)
{
	int rc;
	...
	if (sys_slist_is_empty(&cgms->racp_database)) {
		return generic_handler(peer, cgms, RACP_OPCODE_REPORT_RECS,
				RACP_RESPONSE_NO_RECORDS_FOUND);
	}

	SYS_SLIST_FOR_EACH_NODE(&cgms->racp_database, record_node) {
		entry = CONTAINER_OF(record_node, struct cgms_meas_db_entry, node);
		rc = cgms_racp_send_record(peer, cgms, &entry->meas);
		if (rc != 0) {
	...
	return generic_handler(peer, cgms, RACP_OPCODE_REPORT_RECS, RACP_RESPONSE_SUCCESS);
}

static int report_recs_greater_or_equal_handler(struct bt_conn *peer, struct bt_cgms *cgms,
					struct net_buf_simple *operand)
{
	...
	entry = CONTAINER_OF(sys_slist_peek_tail(&cgms->racp_database),
...
static int report_recs_first_handler(struct bt_conn *peer, struct bt_cgms *cgms)
{
	int rc;
	struct cgms_meas_db_entry *entry;
	...
	entry = CONTAINER_OF(sys_slist_peek_head(&cgms->racp_database),
			struct cgms_meas_db_entry, node);
	rc = cgms_racp_send_record(peer, cgms, &entry->meas);
	if (rc != 0) {
		LOG_WRN("Error occurs when transmitting record: %d", rc);
...
static int report_recs_last_handler(struct bt_conn *peer, struct bt_cgms *cgms)
{
	int rc;
	struct cgms_meas_db_entry *entry;
...
static int report_recs_handler(struct bt_conn *peer, struct bt_cgms *cgms,
			struct net_buf_simple *operators)
{
	int rc = 0;
	...
	case RACP_OPERATOR_ALL:
		rc = report_recs_all_handler(peer, cgms);
		break;
	case RACP_OPERATOR_GREATER_OR_EQUAL:
		rc = report_recs_greater_or_equal_handler(peer, cgms, operators);
		break;
	case RACP_OPERATOR_FIRST:
		rc = report_recs_first_handler(peer, cgms);
		break;
	case RACP_OPERATOR_LAST:
		rc = report_recs_last_handler(peer, cgms);
		break;
	default:
		rc = generic_handler(peer, cgms, RACP_OPCODE_REPORT_RECS,
...
static int report_num_recs_all_handler(struct bt_conn *peer, struct bt_cgms *cgms)
{
	uint16_t count = 0;
	sys_snode_t *record_node;
...
static int report_num_recs_greater_or_equal_handler(struct bt_conn *peer, struct bt_cgms *cgms,
				struct net_buf_simple *operand)
{
	enum racp_operand_filter filter;
	uint16_t time_offset_limit;
...
static int report_num_recs_handler(struct bt_conn *peer, struct bt_cgms *cgms,
			struct net_buf_simple *operators)
{
	enum racp_operator operator;

	if (operators->len < 1) {
		return generic_handler(peer, cgms, RACP_OPCODE_REPORT_NUM_RECS,
			RACP_RESPONSE_INVALID_OPERATOR);
	}

	operator = net_buf_simple_pull_u8(operators);
	if (operator == RACP_OPERATOR_ALL) {
		return report_num_recs_all_handler(peer, cgms);
	}
	if (operator == RACP_OPERATOR_GREATER_OR_EQUAL) {
		return report_num_recs_greater_or_equal_handler(peer, cgms, operators);
	}
...
{
	int rc;
	struct cgms_racp_task *task;
	struct bt_cgms *cgms;
	enum racp_opcode opcode;

	task = CONTAINER_OF(work_item, struct cgms_racp_task, item);
	cgms = CONTAINER_OF(task, struct bt_cgms, racp_task);
	opcode = net_buf_simple_pull_u8(&task->req);

	if (k_mutex_lock(&cgms->racp_lock, K_NO_WAIT) == 0) {
		switch (opcode) {
		case RACP_OPCODE_REPORT_RECS:
			rc = report_recs_handler(task->peer, cgms, &task->req);
			break;
		case RACP_OPCODE_REPORT_NUM_RECS:
			rc = report_num_recs_handler(task->peer, cgms, &task->req);
			break;
		default:
			rc = generic_handler(task->peer, cgms, opcode,
					RACP_RESPONSE_OPCODE_UNSUPPORTED);
			break;
		}
		k_mutex_unlock(&cgms->racp_lock);
	} else {
		rc = -EBUSY;
	}
}

static int abort_handler(struct bt_conn *peer, struct bt_cgms *cgms,
			struct net_buf_simple *operators)
{
	int rc;
	enum racp_operator operator;
	...
	if (operators->len < 1) {
		...
		return generic_handler(peer, cgms, RACP_OPCODE_ABORT_OPERATION,
			RACP_RESPONSE_INVALID_OPERATOR);
	}
	...
	rc = k_work_cancel(&cgms->racp_task.item);
	if (rc == 0) {
		LOG_INF("RACP: work aborted");
		rc = generic_handler(peer, cgms, RACP_OPCODE_ABORT_OPERATION,
...
int cgms_racp_recv_request(struct bt_conn *peer, struct bt_cgms *cgms,
			const uint8_t *req_data, uint16_t req_len)
{
	...
	if (opcode == RACP_OPCODE_ABORT_OPERATION) {
		return abort_handler(peer, cgms, &req);
	}

	/* For other requests, prepare the data and submit to workqueue. */
	if (req_len > sizeof(cgms->racp_task.req_buf)) {
		return -EMSGSIZE;
	}

	cgms->racp_task.peer = peer;
	memcpy(cgms->racp_task.req_buf, req_data, req_len);
	net_buf_simple_init_with_data(&cgms->racp_task.req, cgms->racp_task.req_buf, req_len);
	rc = k_work_submit_to_queue(&racp_work_q, &cgms->racp_task.item);
	LOG_INF("RACP: work submission %s", rc > 0 ? "done" : "failed");
	return rc;
}

int cgms_racp_meas_add(struct bt_cgms *cgms, struct cgms_meas meas)
{
	int rc;
	struct cgms_meas_db_entry *entry;

	if (k_mutex_lock(&cgms->racp_lock, K_NO_WAIT) == 0) {
		entry = entry_alloc(cgms);
		if (entry == NULL) {
			entry = CONTAINER_OF(sys_slist_get(&cgms->racp_database),
					struct cgms_meas_db_entry, node);
		}
		memcpy(&entry->meas, &meas, sizeof(meas));
		sys_slist_append(&cgms->racp_database, &entry->node);
		k_mutex_unlock(&cgms->racp_lock);
		rc = 0;
	} else {
		rc = -EBUSY;
...
int cgms_racp_meas_get_latest(struct bt_cgms *cgms, struct cgms_meas *meas)
{
	int rc;
	struct cgms_meas_db_entry *entry;
...
void cgms_racp_init(struct bt_cgms *cgms)
{
	memset(cgms->racp_pool, 0, sizeof(cgms->racp_pool));

	sys_slist_init(&cgms->racp_database);

	k_mutex_init(&cgms->racp_lock);

	k_work_init(&cgms->racp_task.item, racp_task_handler);

	if (!racp_work_q_started) {
