...
static int generic_handler(struct bt_conn *peer, struct bt_cgms *instance,
		uint8_t req_opcode, uint8_t response_code)
{
	...
	return cgms_socp_send_response(peer, instance, &rsp);
}

static int write_cgms_communication_interval_handler(struct bt_conn *peer,
		struct bt_cgms *instance, struct net_buf_simple *operand)
{
