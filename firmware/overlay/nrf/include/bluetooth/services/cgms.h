...
/** @brief Opaque handle to one Continuous Glucose Monitoring Service instance.
 *
 *  Up to @kconfig{CONFIG_BT_CGMS_INSTANCE_COUNT} instances can be created via
 *  bt_cgms_init(), each with its own GATT service registration, session
 *  state, and measurement database, so each behaves as an independent
 *  virtual sensor.
 */
struct bt_cgms;
...
struct bt_cgms_cb {
	 ...
	 *
	 *  @param cgms The CGMS instance whose session state changed.
	 *  @param session_state The state of current session. True if session starts,
	 *         False if session stops.
	 */
	void (*session_state_changed)(struct bt_cgms *cgms, const bool session_state);
};
 ...
 *
 *  @param[in] cgms The CGMS instance to submit the measurement to.
...
/** @brief Initialize a Continuous Glucose Monitoring service instance.
 *
 *  Allocates and initializes one CGMS instance out of the
 *  @kconfig{CONFIG_BT_CGMS_INSTANCE_COUNT} statically-reserved slots, and
 *  registers its GATT service. Call this once per virtual sensor.
 *
 *  @param[in] init_params Initialization parameters. Must not be NULL.
 *  @param[out] cgms On success, set to the handle for the new instance.
 *              Must not be NULL.
 *
 *  @retval 0 If the operation was successful.
 *          Otherwise, a negative error code is returned.
 */
int bt_cgms_init(const struct bt_cgms_init_param *init_params, struct bt_cgms **cgms);
